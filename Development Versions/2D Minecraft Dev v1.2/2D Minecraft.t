...
var steveAllowMove_x, steveAllowMoveJump, steveOnGround : boolean
...
loop
    ...
    steveFutureFallingHitbox_x1 := steveHitbox_x1 - JUMP_SPEED
    steveFutureFallingHitbox_x2 := steveHitbox_x2 - JUMP_SPEED
	...
	for y : steveFutureHitbox_y1 .. steveFutureHitbox_y2
		...
		steveAllowMoveJump := false
	    end if
	end for
	%Fall Test
	for y : steveFutureFallingHitbox_y1 .. steveFutureFallingHitbox_y1
	    if y >= blockCell_y1 (count) and y <= blockCell_y2 (count) and blockCellType (count) = "air" then
    ...
    %Jumping
    if (chars ('w') or chars (' ')) and steveAllowMoveJump = true then
	vely += JUMP_SPEED
    end if
