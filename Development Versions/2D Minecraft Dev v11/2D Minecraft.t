...
loop
	...
	if mouseClick > 0 and allowBlockDestroy = true then
		...
		if mouse_x >= blockCell_x1 (count) and mouse_x <= blockCell_x2 (count) and mouse_y >= blockCell_y1 (count) and mouse_y <= blockCell_y2 (count) and blockCellType (count) = "TNT"
			...
			if blockCell_x1 (count2) >= blockCell_x1 (count) - 80 and blockCell_x2 (count2) <= blockCell_x2 (count) + 80 and blockCell_y1 (count2) >= blockCell_y1 (count) - 80 and
				blockCell_y2 (count2) <= blockCell_y2 (count) + 80 and blockCellType (count2) ~= "air" and blockCellType (count2) ~= "TNT" blockCellType (count2) ~= "bedrock" then
			    blockCellType (count2) := "air"
			end if
		    end for
	...
	for count : 1 .. 7378
	 ...
	 Pic.Draw (hotbarSlot, 541, 940, picMerge)
	 Pic.Draw (largeDirt, 550, 950, picMerge)
	 Pic.Draw (hotbarSlot, 591, 940, picMerge)
	 Pic.Draw (largeOakLog, 600, 950, picMerge)
	 Pic.Draw (hotbarSlot, 641, 940, picMerge)
	 Pic.Draw (largeOakPlanks, 650, 950, picMerge)
	 Pic.Draw (hotbarSlot, 691, 940, picMerge)
	 Pic.Draw (largeCobblestone, 700, 950, picMerge)
	 Pic.Draw (hotbarSlot, 741, 940, picMerge)
	 Pic.Draw (largeBookshelves, 750, 950, picMerge)
