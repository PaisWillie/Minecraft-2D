...
var loopExitGround, loopExitx : boolean
...
loop
    ...
    %Tests to see if mouse cursor is located near enough to Steve (character) to allow the user to place blocks
    ...
    if mouseClick = 1 and allowBlockPlace = true then
	for count : 1 .. 7378
	    if mouse_x >= blockCell_x1 (count) and mouse_x <= blockCell_x2 (count) and mouse_y >= blockCell_y1 (count) and mouse_y >= blockCell_y2 (count) then
		if blockCellType (count) = "air" then
		end if
	    end for
	end if
	...
	%Steve Draw
	Pic.Draw (steveRight, steve_x, steve_y - 10, picMerge)
